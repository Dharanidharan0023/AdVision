...
const Sidebar = ({ activeTab, setActiveTab, onLogout, isCollapsed, setIsCollapsed }) => {
    ...
    const sidebarContent = (
        <div className={`flex flex-col h-full py-10 ${isCollapsed ? 'px-2' : 'px-4'}`}>
            {/* Admin Header */}
            <div className={`mb-16 flex items-center ${isCollapsed ? 'justify-center px-0' : 'px-6 gap-4'}`}>
                <motion.div 
                    whileHover={{ rotate: 180 }}
                    transition={{ duration: 0.8, ease: "anticipate" }}
                    onClick={() => setIsCollapsed(!isCollapsed)}
                    className="w-12 h-12 rounded-[1.25rem] bg-gradient-to-br from-neon-purple to-neon-cyan flex items-center justify-center text-black shadow-[0_0_20px_rgba(176,38,255,0.4)] cursor-pointer"
            ...
            <nav className="flex-1 space-y-3">
                {navItems.map((item) => {
                    ...
                    return (
                        ...
                        >
                            <div className={`flex items-center relative z-10 ${isCollapsed ? 'justify-center w-full' : 'gap-4'}`}>
                                <Icon 
                                    size={isCollapsed ? 22 : 18} 
                                    className={`transition-all duration-500 ${isActive ? 'scale-110 text-white' : 'group-hover:text-neon-cyan group-hover:scale-110'}`} 
                                />
                                {!isCollapsed && (
            ...
            <div className="pt-8 mt-auto border-t border-white/5 space-y-4">
                <button
                    onClick={onLogout}
                    className={`w-full flex items-center text-red-400/60 hover:text-red-400 hover:bg-red-500/5 rounded-2xl transition-all duration-300 group ${isCollapsed ? 'justify-center py-4 px-0' : 'gap-4 px-6 py-4'}`}
                    title={isCollapsed ? "Purge Session" : ""}
                >
                    <LogOut size={18} className="group-hover:-translate-x-1 transition-transform" />
                    {!isCollapsed && <span className="font-black text-[10px] uppercase tracking-[0.2em]">Purge Session</span>}
                </button>
    ...
    return (
        ...
        >
            ...
            <div className="lg:hidden fixed top-0 left-0 w-full h-20 bg-dark-bg/80 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-6 z-[100]">
                <span className="text-xl font-black neon-text uppercase tracking-tighter">Dharanix Control</span>
                <button 
